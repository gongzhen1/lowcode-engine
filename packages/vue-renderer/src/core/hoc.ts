import { ISlotNode } from '@felce/lowcode-designer';
import { IPublicEnumTransformStage } from '@felce/lowcode-types';
import { isJSSlot } from '@felce/lowcode-utils';
import { isNil } from 'lodash-es';
import { ComponentPublicInstance, defineComponent, Fragment, h, onUnmounted, reactive } from 'vue';
          ...
          if (isRootProp && key) {
            if (key === '___isLocked___') {
              ...
              if (isJSSlot(newValue)) {
                ...
                const schema = slotNode.export(IPublicEnumTransformStage.Render);
