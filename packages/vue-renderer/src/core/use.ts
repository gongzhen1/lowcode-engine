import { Prop } from '@felce/lowcode-designer';
import {
  CompositeValue,
...
import { camelCase, isFunction, isNil, isString, pickBy } from 'lodash-es';
import {
  Component,
  computed,
  ComputedRef,
...
import { getCurrentNodeKey, getRendererContextKey, useRendererContext } from '../hooks';
import {
  BlockScope,
  ensureArray,
  getI18n,
...
export function useLeaf(props: LeafProps) {
   ...
   */
  const buildSchema = () => {
    const schema = node ? node.export(IPublicEnumTransformStage.Render) : props.schema;
    ...
    Object.entries(schema.props ?? {}).forEach(([key, val]) => {
      if (isJSSlot(val)) {
        ...
        if (prop && prop.slotNode) {
          ...
          const slotSchema = prop.slotNode.export(IPublicEnumTransformStage.Render);
  ...
  const buildProp = (
    ...
    if (isJSExpression(schema) || isJSFunction(schema)) {
      ...
      let slotSchema: IPublicTypeNodeData[] | IPublicTypeSlotSchema;
      if (prop?.slotNode) {
        // design 模式，从 prop 中导出 schema
        slotSchema = prop.slotNode.export(IPublicEnumTransformStage.Render);
      ...
      return schema.map((item, idx) => buildProp(item, scope, blockScope, prop?.get(idx)));
  ...
  const buildRefProp = (
    ...
    if (isString(schema)) {
      ...
      return isString(propValue) ? buildRefProp(propValue, scope, blockScope, prop) : propValue;
    }
  };
  ...
  const buildLoop = (schema: IPublicTypeNodeSchema) => {
  ...
  const buildShow = (schema: IPublicTypeNodeSchema) => {
