import { InterpretDataSource, InterpretDataSourceConfig } from '@felce/lowcode-types';
import { isPlainObject } from '@felce/lowcode-utils';
import { isBoolean, isFunction, isUndefined } from 'lodash-es';
import { computed, reactive, ref, shallowRef } from 'vue';
import { parseSchema, RuntimeScope } from '../utils';
...
export function createDataSourceItem(
  ...
  const hooks = {
    ...
    dataHandler: dataHandler ? parseSchema(dataHandler, scope) : (res: Response) => res.data,
  ...
  const load: ExecutionFunc = async (inputParams, otherOptions = {}) => {
    try {
      ...
      const { headers: inputHeaders, assignToScope = true, ...inputOptions } = otherOptions;
      if (!request) {
